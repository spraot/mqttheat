...
class RoomControl():
    ...
    def __init__(self, name, can_heat, can_cool):
        ...
        self.sensors = []
        self.adjacent_rooms = []
        self.pid = PID(200, 1/20, 20000000, setpoint=self.set_temp)
    ...
    def do_onoff(self, temp):
        if self.can_heat and self.mode in ['auto', 'heat'] and temp < self.set_temp:
            self.heating_level = 100
        else:
            self.heating_level = 0

        if self.can_cool and self.mode in ['auto', 'cool'] and temp > self.set_temp + 1:
